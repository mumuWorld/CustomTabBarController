...
/**
 加号按钮label
 */
@property (nonatomic, weak) UILabel *plusLable;
@end
...
{
    if ([self.delegate respondsToSelector:@selector(MMTabBarPlusBtnClick:)]) {
        [self.delegate MMTabBarPlusBtnClick:self];
    }
}
- (void)layoutSubviews
{
    ...
    self.plusLable.text = @"发布";
    self.plusLable.font = [UIFont systemFontOfSize:11];
    [self.plusLable sizeToFit];
    self.plusLable.textColor = [UIColor grayColor];
    self.plusLable.centerX = self.plusBtn.centerX;
    self.plusLable.centerY = CGRectGetMaxY(self.plusBtn.frame) + 10 ;
